...
Important Notes:
1. You should always `set_goal` first, `retrieve_memory`, `retrieve_room` to get possible clues, then decide which object_id to go. If no clues are good, you have four options to find the object: (a) `goToRoom` - use dictionary-based room selection when object is in dictionary, (b) `callLLM` - use LLM reasoning to decide which room, (c) `semanticSimilarityProb` - use CLIP text similarity between target and room labels, or (d) `search_object` - use frontier-based exploration when you need to search systematically.
