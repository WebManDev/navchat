...
class ChatGPTControlAndUserSim(ChatGPTControlVLMap):
    def __init__(self, max_trial, max_round, category, chatgpt_usrsim_config, clear_gptctx=False, *args, **kwargs):
        ...
        assert self.is_vlmap_baseline is True
        assert self.is_cow_baseline is False

        suffix = f"conceptfusion_t{max_trial}r{max_round}_{category}"
